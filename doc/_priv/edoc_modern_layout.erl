...
layout_module(#xmlElement{name = module, content = Es}=E, Opts) ->
    ...
    [{offcanvas,OffcanvasPart}] = ets:lookup(parts, offcanvas),
    ...
    Cont = fun() ->
        [?NL, ?NL, ?NL, {h1, [{class, "uk-heading-small"}], Title}, {hr, [{class, ""}], []}, ?NL]
...
function_index(Fs, Cols) ->
	...
	Rows ->
	    [?NL,
	     {h2, [{id, ?FUNCTION_INDEX_LABEL}],[?FUNCTION_INDEX_TITLE]},
	     ?NL,
	     {'div', [{class, "uk-overflow-auto"}], [
             {table, [{class, "uk-table uk-table-striped"}], Rows}
         ]},
	     ?NL]
    end.
...
label_href(Content, F) ->
	...
	"" -> Content;
	Ref -> [{a, [{href, local_label(Ref)}, {'uk-scroll', ""}], Content}]
    end.
...
functions(Fs, Opts) ->
    ...
    if Es == [] -> [];
       true ->
	    [?NL,
	     {h2, [{id, ?FUNCTIONS_LABEL}], [?FUNCTIONS_TITLE]},
	     ?NL | Es]
    end.

function(Name, E=#xmlElement{content = Es}, Opts) ->
    Card = fun(Content) ->
        [{'div', [{class, "uk-card uk-card-default uk-card-body uk-margin"}], Content}]
    end,
    Card([?NL,
      {h3, [{class, "uk-card-title"}],
       label_function_title(function_header(Name, E, " *"), E)},
      ?NL]
     ++ [{'div',  [{class, "spec"}],
	  [?NL,
	   {code,
...
label_function_title(Content, E) ->
    case get_attrval(label, E) of
	"" -> Content;
	Ref -> [{span, [{id, Ref}], Content}]
    end.
