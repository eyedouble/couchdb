...
json_body(Ref) ->
    ...
    couchdb_ejson:decode(Body).

make_headers(Method, Url, Headers, Options) ->
    Headers1 = case couchdb_util:get_value(<<"Accept">>, Headers) of
        undefined ->
...
maybe_oauth_header(Method, Url, Headers, Options) ->
    case couchdb_util:get_value(oauth, Options) of
        undefined ->
            {Headers, Options};
        OauthProps ->
            Hdr = couchdb_util:oauth_header(Url, Method, OauthProps),
            {[Hdr|Headers], proplists:delete(oauth, Options)}
    end.

maybe_proxyauth_header(Headers, Options) ->
  case couchdb_util:get_value(proxyauth, Options) of
    undefined ->
      {Headers, Options};
    ProxyauthProps ->
...
wait_mp_doc(Ref, Buffer) ->
    ...
    case hackney:stream_multipart(Ref) of
        ...
        end_of_part ->
            %% decode the doc
            {Props} = Doc = couchdb_ejson:decode(Buffer),
            case couchdb_util:get_value(<<"_attachments">>, Props, {[]}) of
                {[]} ->
...
len_doc_to_mp_stream(Atts, Boundary, {Props}) ->
    ...
    Doc1 = case couchdb_util:get_value(<<"_attachments">>, Props) of
        undefined ->
        ...
        {OldAtts} ->
            ...
            OldAtts1 = lists:foldl(fun({Name, AttProps}, Acc) ->
                            case couchdb_util:get_value(Name, Stubs) of
                                undefined ->
                                    [{Name, AttProps} | Acc];
                                _ ->
                                    Acc
    ...
    %% eencode the doc
    JsonDoc = couchdb_ejson:encode(Doc1),
...
mp_doc_reply(Ref, Doc) ->
    ...
    case couchdb_httpc:db_resp(Resp, [200, 201]) of
        {ok, _, _, Ref} ->
            ...
            %% set the new doc ID
            Doc1 = couchdb_doc:set_value(<<"_id">>, NewDocId, Doc),
            %% set the new rev
            FinalDoc = couchdb_doc:set_value(<<"_rev">>, NewRev, Doc1),
