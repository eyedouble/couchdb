...
%% <p>See {@link couchdb_view:stream/4} for more information about
%% options.</p>
...
stream(Db, ViewName, Options) ->
    ...
    make_view(Db, ViewName, Options1, fun(Args, Url) ->
                Ref = make_ref(),
                Req = {Db, Url, Args},
                case supervisor:start_child(couchdb_view_sup, [To,
                                                                 Ref,
                                                                 Req,
                                                                 Options]) of
                    {ok, _Pid} ->
...
count(Db, ViewName, Options)->
    ...
    Options1 = couchdb_util:force_param(limit, 0, Options),
    ...
    make_view(Db, ViewName, Options1, fun(Args, Url) ->
                case view_request(Db, Url, Args) of
                    {ok, _, _, Ref} ->
                        ...
                        couchdb_util:get_value(<<"total_rows">>, Props);
                    Error ->
                        Error
                end
    end).
...
first(Db, ViewName, Options) ->
    ...
    %% all the results
    Options1 = couchdb_util:force_param(limit, 1, Options),
    ...
    make_view(Db, ViewName, Options1, fun(Args, Url) ->
                case view_request(Db, Url, Args) of
                    {ok, _, _, Ref} ->
                        {Props} = couchdb_httpc:json_body(Ref),
                        case couchdb_util:get_value(<<"rows">>, Props) of
                            [] ->
                                {ok, nil};
                            [Row] ->
                                {ok, Row}
...
%% ```
%% couchdb_view:fold(fun(Row, Acc) -> [Row|Acc] end, [], Db, 'all_docs').
%% '''
fold(Function, Acc, Db, ViewName, Options) ->
    %% make sure we stream item by item so we can stop at any time.
    Options1 = couchdb_util:force_param(async, once, Options),
...
%% ```
%% couchdb_view:foreach(fun(Row) -> io:format("got row ~p~n", [Row]) end, Db, 'all_docs').
%% '''
...
parse_view_options([{key, Value}|Rest], #view_query_args{options=Opts}=Args) ->
    Opts1 = [{key, couchdb_ejson:encode(Value)}|Opts],
...
make_view(#db{server=Server}=Db, ViewName, Options, Fun) ->
    ...
    case ViewName of
        ...
        {DName, VName} ->
            Url = hackney_url:make_url(couchdb_httpc:server_url(Server),
                                       [couchdb_httpc:db_url(Db), <<"_design">>,
                                        DName, <<"_view">>, VName],
                                       Args#view_query_args.options),
            Fun(Args, Url);
        _ ->
...
view_request(#db{options=Opts}, Url, Args) ->
    case Args#view_query_args.method of
        get ->
            couchdb_httpc:db_request(get, Url, [], <<>>,
                                       Opts, [200]);
        post ->
            Body = couchdb_ejson:encode(
                    {[{<<"keys">>, Args#view_query_args.keys}]}
                    ),

            Hdrs = [{<<"Content-Type">>, <<"application/json">>}],
            couchdb_httpc:db_request(post, Url, Hdrs, Body,
                                       Opts, [200])
    end.

with_view_stream(Ref, Fun) ->
    case ets:lookup(couchdb_view_streams, Ref) of
        [] ->
