...
init_stream(Parent, Owner, StreamRef, {_Db, _Url, _Args}=Req,
    ...
    case do_init_stream(Req, InitState) of
        {ok, State} ->
            %% register the stream
            ets:insert(couchdb_view_streams, [{StreamRef, self()}]),
            %% start the loop
            loop(State);
        Error ->
...
do_init_stream({#db{options=Opts}, Url, Args}, #state{mref=MRef}=State) ->
    ...
    Reply = case Args#view_query_args.method of
        get ->
            couchdb_httpc:request(get, Url, [], <<>>, FinalOpts);
        post ->
            Body = couchdb_ejson:encode({[{<<"keys">>,
                                             Args#view_query_args.keys}]}),
            Headers = [{<<"Content-Type">>, <<"application/json">>}],
            couchdb_httpc:request(post, Url, Headers, Body, FinalOpts)
    end,
...
loop(#state{owner=Owner,
    ...
    receive
        ...
        {hackney_response, ClientRef, done} ->
            ...
            ets:delete(couchdb_view_streams, StreamRef),
...
maybe_continue(#state{parent=Parent, owner=Owner, ref=Ref, mref=MRef,
    ...
    receive
        ...
        {Ref, cancel} ->
            maybe_close(State),
            %% unregister the stream
            ets:delete(couchdb_view_streams, Ref),
...
system_terminate(Reason, _, _, #state{ref=StreamRef,
    ...
    %% unregister the stream
    catch ets:delete(couchdb_view_streams, StreamRef),
    exit(Reason).
...
send_row(Row, #viewst{owner=Owner, ref=Ref}=ViewSt) ->
    Owner ! {Ref, {row, couchdb_ejson:post_decode(Row)}},
