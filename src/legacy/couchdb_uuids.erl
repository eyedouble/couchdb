...
%%--------------------------------------------------------------------
%% @doc Starts the couchdb process linked to the calling process. Usually
%% invoked by the supervisor couchdb_sup
...
do_get_uuids(Server, Count, Acc, [#server_uuids{uuids=Uuids}]) ->
    case Uuids of
        ...
        _ ->
            ...
            ets:insert(couchdb_uuids, ServerUuids),
...
get_new_uuids(#server{url=ServerUrl, options=Opts}=Server, Backoff, Acc) ->
    ...
    case couchdb_httpc:request(get, Url, [], <<>>, Opts) of
        {ok, 200, _, Ref} ->
            {ok, Body} = hackney:body(Ref),
            {[{<<"uuids">>, Uuids}]} = couchdb_ejson:decode(Body),
